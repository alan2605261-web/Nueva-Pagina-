...
export default function GarantiaPage() {
  return (
    <PageShell>
        ...
        <div className="container-edge">
          <Reveal>
            <article className="container-prose">
              ...
              <ul className="mb-4 list-disc space-y-2 pl-6 text-ink-soft body-lg">
                <li>
      ...
      {/* ── MF ONE ─────────────────────────────────────────── */}
      <section id="mf-one" className="section-y bg-warm scroll-mt-20">
        <div className="container-edge">
          <Reveal>
            <article className="container-prose">
              <p className="eyebrow mb-3">Garantía limitada · MF ONE · CP-ONE</p>
              <h2 className="display-md mb-6">Doce meses, desde que la recibes</h2>
              ...
              <h3 className="heading-sm mb-4">Qué componentes ampara</h3>
              <ul className="mb-4 list-disc space-y-2 pl-6 text-ink-soft body-lg">
                {ONE_CUBRE.map((c) => (
                  <li key={c}>{c}</li>
                ))}
              ...
              <h3 className="heading-sm mb-4">Qué corre por nuestra cuenta</h3>
              <dl className="mb-10 divide-y divide-line border-y border-line">
                {ONE_POR_NUESTRA_CUENTA.map(([k, v]) => (
                  ...
                  >
                    <dt className="text-ink-soft body-lg">{k}</dt>
                    <dd className="text-sm font-semibold text-foreground">{v}</dd>
                  </div>
                ))}
              </dl>

              <div className="mb-10 rounded-xl border border-line p-6">
                <p className="eyebrow mb-3">Condición de validez</p>
                <p className="body-lg text-ink-soft">
              ...
              <h3 className="heading-sm mb-4">Qué no cubre</h3>
              ...
              {ONE_NO_CUBRE.map((g) => (
                <div key={g.grupo} className="mb-8">
                  <p className="eyebrow mb-3">{g.grupo}</p>
                  <ul className="list-disc space-y-2 pl-6 text-ink-soft body-lg">
                    {g.items.map((i) => (
                      <li key={i}>{i}</li>
                    ))}
                  </ul>
              ...
              <div className="mb-10 rounded-xl border border-line p-6">
                <p className="eyebrow mb-3">Uso comercial</p>
                <p className="body-lg text-ink-soft">
              ...
              <h3 className="heading-sm mb-6">Cómo se hace válida</h3>
              <ol className="mb-10 space-y-6">
                {ONE_PROCESO.map((p) => (
                  <li key={p.n} className="flex gap-5">
                    <span className="text-2xl font-light text-ink-faint">
                      {p.n}
                    </span>
                    <div>
                      <p className="font-semibold text-foreground">{p.t}</p>
                      <p className="body-lg mt-1 text-ink-soft">{p.d}</p>
                    </div>
                  </li>
                ))}
              </ol>

              <h3 className="heading-sm mb-4">Los datos de tu equipo</h3>
              <dl className="mb-4 divide-y divide-line border-y border-line">
                {[
      ...
      {/* ── Inflables ──────────────────────────────────────── */}
      <section id="inflables" className="section-y bg-background scroll-mt-20">
        <div className="container-edge">
          <Reveal>
            <article className="container-prose">
              <p className="eyebrow mb-3">Garantía limitada · MF Barrel y MF Horizon</p>
              <h2 className="display-md mb-6">Seis meses en los modelos inflables</h2>
      ...
      <section className="section-y bg-warm">
        <div className="container-edge">
          <Reveal>
            <article className="container-prose">
              <h2 className="heading-sm mb-4">Además: 30 días de prueba</h2>
              <p className="body-lg text-ink-soft">
